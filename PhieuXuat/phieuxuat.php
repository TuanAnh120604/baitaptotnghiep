...
try {
    ...
    if ($role === 'Thủ kho' && $ma_nd) {
        $where_sql .= ' AND px.ma_kho IN (SELECT ma_kho FROM kho WHERE ma_nd = :ma_nd)';
        $params[':ma_nd'] = $ma_nd;
    } elseif ($role === 'Quản lý kho' && $ma_nd) {
        $where_sql .= ' AND px.ma_kho IN (
            SELECT k.ma_kho 
            FROM kho k 
    ...
    // Lọc theo ngày xuất
    if ($ngay !== '') {
        $where_sql .= ' AND DATE(px.ngay_xuat) = :ngay';
        $params[':ngay'] = $ngay;
    }
...
<body class="bg-background-light dark:bg-background-dark text-gray-900 dark:text-gray-100 font-body antialiased transition-colors duration-200">
                                        ...
                                        <td class="px-4 py-4 text-right sticky right-0 bg-white dark:bg-surface-dark group-hover:bg-gray-50 dark:group-hover:bg-transparent shadow-l">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <?php if ($role === 'Thủ kho' && ($trang_thai ?? 'cho_xac_nhan') === 'cho_xac_nhan'): ?>
                                                    <button onclick="xacNhanPhieu('xuat', '<?= htmlspecialchars($row['ma_phieu_xuat']) ?>')"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <?php if ($total_pages > 1): ?>
                        <div class="flex items-center gap-1">
                            ...
                            $prev_url = 'phieuxuat.php?' . http_build_query(array_merge($query_params, ['page' => $prev_page]));
                            ?>
                            ...
                            if ($start_page > 1): ?>
                                <a href="phieuxuat.php?<?= http_build_query(array_merge($query_params, ['page' => 1])) ?>"
                            ...
                            <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                                <a href="phieuxuat.php?<?= http_build_query(array_merge($query_params, ['page' => $i])) ?>"
                            ...
                            if ($end_page < $total_pages): ?>
                                <?php if ($end_page < $total_pages - 1): ?>
                                    <span class="h-8 w-8 flex items-center justify-center text-gray-600 dark:text-gray-300">...</span>
                                <?php endif; ?>
                                <a href="phieuxuat.php?<?= http_build_query(array_merge($query_params, ['page' => $total_pages])) ?>"
                                    class="h-8 w-8 flex items-center justify-center rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm font-medium">
                                    <?= $total_pages ?>
                                </a>
                            <?php endif; ?>
                            ...
                            $next_url = 'phieuxuat.php?' . http_build_query(array_merge($query_params, ['page' => $next_page]));
                            ?>
    ...
    <script>
        ...
        const dateInput = document.querySelector('input[name="ngay"]');
        ...
        input.addEventListener('input', () => {
            clearTimeout(timer);
            timer = setTimeout(searchPhieuXuat, 300);
        });
