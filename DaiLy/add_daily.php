...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_agent') {
    ...
    if (empty($ten_dai_ly) || empty($dia_chi) || empty($sdt)) {
        ...
        try {
            ...
            $check_sdt = $pdo->prepare('SELECT COUNT(*) as count FROM dai_ly WHERE sdt = ?');
            $check_sdt->execute([$sdt]);
            if ($check_sdt->fetch(PDO::FETCH_ASSOC)['count'] > 0) {
                header('Location: daily.php?status=error&message=' . urlencode('Số điện thoại này đã tồn tại'));
                exit();
            }

            // Kiểm tra CCCD không được trùng (nếu có nhập)
            if (!empty($cccd)) {
                $check_cccd = $pdo->prepare('SELECT COUNT(*) as count FROM dai_ly WHERE cccd = ?');
                $check_cccd->execute([$cccd]);
                if ($check_cccd->fetch(PDO::FETCH_ASSOC)['count'] > 0) {
                    header('Location: daily.php?status=error&message=' . urlencode('CCCD/CMND này đã tồn tại'));
                    exit();
                }
            }
