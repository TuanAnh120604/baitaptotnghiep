...
try {
    // Đếm tổng số bản ghi
    $count_stmt = $pdo->prepare('SELECT COUNT(*) as total FROM dai_ly');
    $count_stmt->execute();
    $total_items = $count_stmt->fetch(PDO::FETCH_ASSOC)['total'];
    $total_pages = ceil($total_items / $items_per_page);

    // Lấy dữ liệu phân trang
    $stmt = $pdo->prepare('SELECT * FROM dai_ly ORDER BY ma_dai_ly LIMIT ? OFFSET ?');
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-8 bg-background-light dark:bg-background-dark">
            ...
            <div class="bg-surface-light dark:bg-surface-dark shadow-sm rounded-xl border border-border-light dark:border-border-dark overflow-hidden transition-colors duration-200">
                                            ...
                                            <div class="flex items-center justify-end space-x-2">
                                                ...
                                                <?php if (canDelete('daily')): ?>
                                                    <form method="POST" action="delete_daily.php" style="display:inline;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa đại lý này không?');">
                                                        ...
                                                        <button type="submit" class="p-1.5 rounded-md text-[#637588] hover:text-red-500 hover:bg-red-50" title="Xóa">
                ...
                <!-- Phân trang -->
                <div class="px-6 py-4 border-t border-border-light dark:border-border-dark flex items-center justify-between bg-surface-light dark:bg-surface-dark">
                    ...
                    <?php if ($total_pages > 1): ?>
                        <div class="flex items-center gap-1">
                            ...
                            $prev_url = 'daily.php?' . http_build_query(['page' => $prev_page]);
                            ?>
                            ...
                            if ($start_page > 1): ?>
                                <a href="daily.php?<?= http_build_query(['page' => 1]) ?>"
                            ...
                            <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                                <a href="daily.php?<?= http_build_query(['page' => $i]) ?>"
                            ...
                            if ($end_page < $total_pages): ?>
                                <?php if ($end_page < $total_pages - 1): ?>
                                    <span class="h-8 w-8 flex items-center justify-center text-gray-600 dark:text-gray-300">...</span>
                                <?php endif; ?>
                                <a href="daily.php?<?= http_build_query(['page' => $total_pages]) ?>"
                                    class="h-8 w-8 flex items-center justify-center rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm font-medium">
                                    <?= $total_pages ?>
                                </a>
                            <?php endif; ?>
                            ...
                            $next_url = 'daily.php?' . http_build_query(['page' => $next_page]);
                            ?>
