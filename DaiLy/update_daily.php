...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_agent') {
    ...
    if (empty($ma_dai_ly) || empty($ten_dai_ly) || empty($dia_chi) || empty($sdt)) {
        ...
        try {
            ...
            $check_sdt = $pdo->prepare('SELECT COUNT(*) as count FROM dai_ly WHERE sdt = ? AND ma_dai_ly != ?');
            $check_sdt->execute([$sdt, $ma_dai_ly]);
            if ($check_sdt->fetch(PDO::FETCH_ASSOC)['count'] > 0) {
                header('Location: daily.php?status=error&message=' . urlencode('Số điện thoại này đã tồn tại ở đại lý khác'));
                exit();
            }

            // Kiểm tra CCCD không được trùng (nếu có nhập, bỏ qua chính nó)
            if (!empty($cccd)) {
                $check_cccd = $pdo->prepare('SELECT COUNT(*) as count FROM dai_ly WHERE cccd = ? AND ma_dai_ly != ?');
                $check_cccd->execute([$cccd, $ma_dai_ly]);
                if ($check_cccd->fetch(PDO::FETCH_ASSOC)['count'] > 0) {
                    header('Location: daily.php?status=error&message=' . urlencode('CCCD/CMND này đã tồn tại ở đại lý khác'));
                    exit();
                }
            }
