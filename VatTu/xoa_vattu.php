...
if (!canDelete('vattu')) {
    header('Location: vattu.php?error=Không có quyền xóa vật tư');
    exit;
}

if ($_SERVER['REQUEST_METHOD'] !== 'GET' || !isset($_GET['ma_hang'])) {
    header('Location: vattu.php?error=' . urlencode('Thiếu thông tin mã hàng'));
    exit;
}
...
if ($ma_hang === '') {
    ...
    header('Location: vattu.php?' . http_build_query($redirect_params));
    exit;
}
...
try {
    ...
    // Đếm lại tổng số bản ghi sau khi xóa
    $items_per_page = 10;
    if ($q !== '') {
        ...
        ");
        $like = "%$q%";
        $count_after_stmt->execute([$like, $like]);
    } else {
        $count_after_stmt = $pdo->prepare("
            SELECT COUNT(*) as total
            FROM hang_hoa
            WHERE ma_loai_hang != 'M004'
        ");
        $count_after_stmt->execute();
    }
    $total_items_after = $count_after_stmt->fetch(PDO::FETCH_ASSOC)['total'];
    $total_pages_after = ceil($total_items_after / $items_per_page);

    // Xác định trang redirect
    $redirect_page = $page;
    if ($total_pages_after > 0) {
        // Nếu trang hiện tại lớn hơn số trang tối đa sau khi xóa, chuyển về trang cuối cùng
        if ($page > $total_pages_after) {
            $redirect_page = $total_pages_after;
        }
    } else {
        // Nếu không còn dữ liệu, về trang 1
        $redirect_page = 1;
    }
