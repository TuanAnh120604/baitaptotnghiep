...
<head>
    ...
    <title>Mặt hàng - Thành phẩm</title>
...
<body class="bg-background-light dark:bg-background-dark text-[#111418] font-display antialiased overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto p-6 md:p-8">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-medium">
                                <?php if (isset($_GET['deleted'])): ?>
                                    Đã xóa thành phẩmphẩm "<?= htmlspecialchars($_GET['deleted']) ?>" thành công!
                                <?php else: ?>
                                    Thêm thành phẩm thành công!
                                <?php endif; ?>
                            </span>
                        </div>
                    </div>
                ...
                <?php if (isset($_GET['updated'])): ?>
                    <div id="updatedMessage"
                        ...
                        <div class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-green-600 dark:text-green-400">check_circle</span>
                            <span class="font-medium">Cập nhật thành phẩm thành công!</span>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div class="max-w-[1200px] mx-auto flex flex-col gap-6">
                    ...
                    <div
                        ...
                        <form id="searchForm" class="flex flex-col lg:flex-row gap-4"></form>
                        <div class="flex-1 relative">
                            ...
                            <input id="searchInput" name="q" value="<?= htmlspecialchars($q) ?>"
                                ...
                                placeholder="Tìm kiếm theo mã hoặc tên thành phẩm..." type="text"
                                autocomplete="off" />
                        </div>
                        </form>
                    </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr
                                        ...
                                        <th
                                            class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-[#637588] dark:text-[#9ca3af] w-[15%]">
                                            Mã TP</th>
                                        <th
                                ...
                                <tbody id="tableBody" class="divide-y divide-[#e5e7eb] dark:divide-[#2a3b4d]">
                                    ...
                                    <?php if (empty($thanh_pham_list)): ?>
                                        <tr>
                                            <td colspan="5" class="py-6 text-center text-gray-500">Chưa có thành phẩm nào.
                                            </td>
                                        </tr>
                                    <?php else: ?>
                        ...
                        <div id="paginationContainer"
                            ...
                            <span class="text-sm text-[#637588] dark:text-[#9ca3af]">
                                <?php if ($total_records > 0): ?>
                                    Hiển thị <?= $start_record ?>-<?= $end_record ?> trên <?= $total_records ?> kết quả
                                <?php else: ?>
                                    Không có kết quả
                                <?php endif; ?>
                            </span>
    ...
    <div id="addModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div
            ...
            <div class="flex items-center justify-between p-5 border-b border-[#e5e7eb] dark:border-[#2a3b4d]">
                <h3 class="text-lg font-bold text-[#111418] dark:text-white">Thêm thành phẩm mới</h3>
            ...
            <div class="p-6 overflow-y-auto">
                <form method="POST" action="luu_thanhpham.php" class="flex flex-col gap-4" novalidate>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm font-medium text-[#111418] dark:text-white">Loại hàng</label>
                        <input type="text" value="Thành phẩm" disabled
                            class="h-10 px-3 rounded-lg border border-[#dce0e5] bg-gray-100 text-[#111418] text-sm cursor-not-allowed" />
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div
            ...
            <div class="p-6 overflow-y-auto">
                <form method="POST" action="capnhat_thanhpham.php" id="editForm" class="flex flex-col gap-4" novalidate>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input name="ten_hang" id="edit_ten_hang" required
                            class="h-10 px-3 rounded-lg border border-[#dce0e5] dark:border-[#2a3b4d] bg-white dark:bg-[#0d141c] text-[#111418] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-[#637588] dark:placeholder:text-[#64748b]"
                            type="text" />
                        <p id="edit_ten_hang_error" class="mt-1 text-sm text-red-600 dark:text-red-400 hidden">Vui lòng nhập tên thành phẩm</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                            <label class="text-sm font-medium text-[#111418] dark:text-white">Đơn giá <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <input name="don_gia" id="edit_don_gia" required
                                    class="w-full h-10 pl-3 pr-8 rounded-lg border border-[#dce0e5] dark:border-[#2a3b4d] bg-white dark:bg-[#0d141c] text-[#111418] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-[#637588] dark:placeholder:text-[#64748b]"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex flex-col gap-1.5">
                            <label class="text-sm font-medium text-[#111418] dark:text-white">Mức dự trù Min <span class="text-red-500">*</span></label>
                            <input name="muc_du_tru_min" id="edit_muc_du_tru_min" type="number" min="0" step="0.01" required
                                class="h-10 px-3 rounded-lg border border-[#dce0e5] dark:border-[#2a3b4d] bg-white dark:bg-[#0d141c] text-[#111418] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-[#637588] dark:placeholder:text-[#64748b]"
                        ...
                        <div class="flex flex-col gap-1.5">
                            <label class="text-sm font-medium text-[#111418] dark:text-white">Mức dự trù Max <span class="text-red-500">*</span></label>
                            <input name="muc_du_tru_max" id="edit_muc_du_tru_max" type="number" min="0" step="0.01" required
                                class="h-10 px-3 rounded-lg border border-[#dce0e5] dark:border-[#2a3b4d] bg-white dark:bg-[#0d141c] text-[#111418] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-[#637588] dark:placeholder:text-[#64748b]"
    ...
    <script>
        ...
        function closeModal() {
            ...
            if (form) {
                form.reset();
                hideError('add_ten_hang', 'add_ten_hang_error');
                hideError('add_don_vi_tinh', 'add_don_vi_tinh_error');
        ...
        if (editForm) {
            ...
            if (editTenHang) {
                editTenHang.addEventListener('blur', () => validateRequired('edit_ten_hang', 'edit_ten_hang_error', 'Vui lòng nhập tên thành phẩm'));
            }
            if (editDonViTinh) {
            ...
            editForm.addEventListener('submit', function(e) {
                ...
                // Kiểm tra tất cả các trường bắt buộc
                if (editTenHang && !validateRequired('edit_ten_hang', 'edit_ten_hang_error', 'Vui lòng nhập tên thành phẩm')) {
                    isValid = false;
                }
        ...
        // Tự động tìm kiếm khi gõ - không cần nhấn Enter + phân trang ajax
        let searchTimeout;
        ...
        function searchTable() {
            ...
            if (tableBody) {
                tableBody.innerHTML = '<tr><td colspan="5" class="py-6 text-center text-gray-500">Đang tải...</td></tr>';
            }
            if (paginationContainer) {
                paginationContainer.innerHTML = '';
            }

            // Tạo URL với tham số tìm kiếm + phân trang
            ...
            fetch('ajax_get_table.php?' + params.toString())
                ...
                .then(data => {
                    if (data.success && tableBody) {
                        ...
                        if (paginationContainer && typeof data.pagination_html !== 'undefined') {
                            paginationContainer.innerHTML = data.pagination_html;
                        }

                        // Cập nhật URL mà không reload
                        const urlParams = new URLSearchParams();
                        if (searchValue) urlParams.append('q', searchValue);
                        if (currentPage > 1) urlParams.append('page', currentPage);
                        const newUrl = window.location.pathname + (urlParams.toString() ? '?' + urlParams.toString() : '');
                        window.history.pushState({}, '', newUrl);
                    } else if (tableBody) {
                        tableBody.innerHTML = '<tr><td colspan="5" class="py-6 text-center text-red-600 dark:text-red-400">' + (data.error || 'Có lỗi xảy ra') + '</td></tr>';
                ...
                .catch(error => {
                    ...
                    if (tableBody) {
                        tableBody.innerHTML = '<tr><td colspan="5" class="py-6 text-center text-red-600 dark:text-red-400">Có lỗi xảy ra khi tải dữ liệu</td></tr>';
                    }
                    if (paginationContainer) paginationContainer.innerHTML = '';
                });
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Lưu nội dung bảng & phân trang ban đầu
            if (tableBody) originalTableContent = tableBody.innerHTML;
            if (paginationContainer) originalPaginationContent = paginationContainer.innerHTML;
            ...
            if (searchInput) {
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    if (searchValue === '') {
                        if (tableBody && originalTableContent) tableBody.innerHTML = originalTableContent;
                        if (paginationContainer && originalPaginationContent) paginationContainer.innerHTML = originalPaginationContent;
                        currentPage = 1;
            ...
            if (updatedMessage) {
                setTimeout(function() {
                    ...
                    updatedMessage.style.transform = 'translateX(100%)';
                    setTimeout(function() {
            ...
            // Xóa tham số thông báo khỏi URL sau khi hiển thị để tránh hiển thị lại khi refresh
            const params = new URLSearchParams(window.location.search);
            let urlChanged = false;
            ['success', 'deleted', 'error', 'updated'].forEach(key => {
                if (params.has(key)) {
                    params.delete(key);
                    urlChanged = true;
                }
        ...
        // Chuyển trang khi dùng pagination ajax
