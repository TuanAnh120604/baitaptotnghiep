...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        if ($user && password_verify($password, $user['mat_khau'])) {
            ...
            // Nếu là Quản lý kho, lấy thông tin phân quyền
            if (trim($user['ten_vai_tro']) === 'Quản lý kho') {
                $sql_perm = "
                    SELECT pq.ma_vung, pq.ma_loai_kho, vm.ten_vung, lk.ten_loai_kho
                    FROM phan_quyen pq
                ...
                $stmt_perm->execute(['ma_nd' => $user['ma_nd']]);
                $perm = $stmt_perm->fetch(PDO::FETCH_ASSOC);

                if ($perm) {
                    $_SESSION['ma_vung'] = $perm['ma_vung'];
                    $_SESSION['ten_vung'] = $perm['ten_vung'];
                    $_SESSION['ma_loai_kho'] = $perm['ma_loai_kho'];
                    $_SESSION['ten_loai_kho'] = $perm['ten_loai_kho'];
                }
            }

            // Redirect based on role
            if (trim($user['ten_vai_tro']) === 'Thủ kho') {
                header('Location: ../TheKho/thekho.php');
            } else {
